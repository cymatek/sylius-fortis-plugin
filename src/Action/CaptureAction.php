...
final class CaptureAction implements ActionInterface, ApiAwareInterface, GatewayAwareInterface
{
    ...
    public function execute($request): void
    {
        ...
        $resp = match ($d['payment_method'] ?? 'token') {
            'token' => $api->saleTokenized($d['token_id'], $d['amount_minor'], $d['order_number'] ?? null, $d['location_id'] ?? null),
            'keyed' => $api->saleKeyed($d['account_number'], $d['exp_date'], $d['cvv'] ?? null, $d['amount_minor'], $d['order_number'] ?? null, $d['location_id'] ?? null),
