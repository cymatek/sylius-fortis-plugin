...
// Typed request models (your SDK variant expects positional constructors)
...
final class FortisSdkAdapter implements FortisApiInterface
{
                ...
                if ($type instanceof \ReflectionUnionType) {
                    foreach ($type->getTypes() as $t) {
                        ...
                        if (!$t->isBuiltin()) {
                            $cls = $t->getName();
                            if (class_exists($cls)) {
                                if (method_exists($cls, 'from')) {
                                    return $b->{$method}($cls::from($value));
                                }
                                if (method_exists($cls, 'of')) {
                                    return $b->{$method}($cls::of($value));
                                }
    ...
    private function warnOnceFallback(): void
    {
        if (!defined('FORTIS_FALLBACK_WARNED')) {
            define('FORTIS_FALLBACK_WARNED', true);
            @error_log('[FortisSdkAdapter] SDK auth failed; using HTTP fallback. Set FORTIS_SDK_STRICT=1 to fail.');
        }
    }
